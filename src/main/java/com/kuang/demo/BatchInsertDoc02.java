...
public class BatchInsertDoc02 {
    ...
    public static void main(String[] args) {
        ConnectElasticsearch.connect(client -> {
            ...
            request.add(new IndexRequest().index("user").id("1006").source(XContentType.JSON, "name", "wangwu4", "age", 30, "sex", "男"));
            request.add(new IndexRequest().index("user").id("1007").source(XContentType.JSON, "name", "wangwu44", "age", 40, "sex", "男"));
            request.add(new IndexRequest().index("user").id("1008").source(XContentType.JSON, "name", "wangwu4444", "age", 60, "sex", "男"));
            //客户端发送请求，获取响应对象
